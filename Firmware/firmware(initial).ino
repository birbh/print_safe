...
void fingerenroll(){
    ...
    uint8_t enrollresult = getFingerprintEnroll(fingerID);
    //enroll 
    if (enrollresult == FINGERPRINT_OK) {
        showmsg("ENROLLED OK!", "SLOT ID: " + String(fingerID));
        fingerID++; 
        successbeep();
    } else {
        showmsg("ENROLL FAILED", "TRY AGAIN");
        warningbeep();
    }
    delay(4000);
}
...
uint8_t getFingerprintEnroll(int id) {
  ...
  showmsg("Scanning","Enroll id: " + String(id));
